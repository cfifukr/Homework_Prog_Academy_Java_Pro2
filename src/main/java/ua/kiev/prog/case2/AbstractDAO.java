...
public abstract class AbstractDAO<T> {
    ...
    public void add(T t) {
        try {
            ...
            String sqlId = "SELECT count(*) as numberUsers FROM " + table;

            try (Statement st = conn.createStatement()) {
                ResultSet rs = st.executeQuery(sqlId);
                rs.next();
                id.setAccessible(true);
                id.set(t, rs.getInt("numberUsers"));
    ...
    public List<T> getAll(Class<T> cls, String ... columns) {
        ...
        if(columns.length != 0){
            ...
            sqlRequest.deleteCharAt(sqlRequest.length()-2);
        }else{
            sqlRequest.append( "* ");
        }
        sqlRequest.append("FROM " + table);
        try {
            try (Statement st = conn.createStatement()) {

                try (ResultSet rs = st.executeQuery(sqlRequest.toString())) {
